...
static ssize_t fpga_err_info_read(struct file *file, char __user *buf, size_t count, loff_t *p_pos)
{
	...
	ret = simple_read_from_buffer(buf, count, p_pos, &mnt_pri->excep_type, sizeof(mnt_pri->excep_type));
	FPGA_INFO("fpga_poll_wakeup %llx\r\n",mnt_pri->excep_type);
	mnt_pri->excep_type = 0;
