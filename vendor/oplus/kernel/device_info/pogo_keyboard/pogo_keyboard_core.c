...
/*
dfu ota thread
1.send bat file info
2.send bat file data
3.send bat file end
4.send bin file info
5.send bin file data
6.send bin file end
basic info: 64byte
fw info: (64byte)
         ID,file type(2byte),
...
//for dfu ota
static int kpd_fw_dfu_isvalid(const unsigned char *fw_data, u32 count, u32 *checksum1, u32 *addr1,
        u32 *len1, int *type1, u32 *checksum2, u32 *addr2, u32 *len2, int *type2)
{
    unsigned char bat_fwinfo[DFU_FW_INFO_LEN] ={0};
    unsigned char bin_fwinfo[DFU_FW_INFO_LEN] ={0};
    u32 crc32 = 0;
    u32 fwinfo_addr = 0;
    u32 bat_start_addr = 0;
    u32 bat_len = 0;
    int bat_fw_version = 0;
    u32 bin_start_addr = 0;
    u32 bin_len = 0;
    int bin_fw_version = 0;
    u8 get_ver_addr = 0;
    ...
    memcpy(bat_fwinfo, &fw_data[fwinfo_addr], sizeof(bat_fwinfo));
    bat_start_addr = bat_fwinfo[2] << 24 | bat_fwinfo[3] << 16 | bat_fwinfo[4] << 8 | bat_fwinfo[5];
    bat_len = bat_fwinfo[6] << 24 | bat_fwinfo[7] << 16 | bat_fwinfo[8] << 8 | bat_fwinfo[9];
    *addr1 = bat_start_addr;
    *len1 = bat_len;
    *type1 = bat_fwinfo[0] << 8 | bat_fwinfo[1];
    kb_debug("%s %d bat_start_addr: 0x%x, bat_len:0x%x, type:0x%x\n", __func__, __LINE__, *addr1, *len1, *type1);

    memcpy(bin_fwinfo, &fw_data[fwinfo_addr + DFU_FW_INFO_LEN], sizeof(bin_fwinfo));
    bin_start_addr = bin_fwinfo[2] << 24 | bin_fwinfo[3] << 16 | bin_fwinfo[4] << 8 | bin_fwinfo[5];
    bin_len = bin_fwinfo[6] << 24 | bin_fwinfo[7] << 16 | bin_fwinfo[8] << 8 | bin_fwinfo[9];
    *addr2 = bin_start_addr;
    *len2 = bin_len;
    *type2 = bin_fwinfo[0] << 8 | bin_fwinfo[1];
    kb_debug("%s %d bin_start_addr: 0x%x, bin_len:0x%x, type:0x%x\n", __func__, __LINE__, *addr2, *len2, *type2);

    if ((bin_start_addr + bin_len) > count) {
        kb_err("%s %d ota file count < bin_start_addr + bin_len, please check!!!\n", __func__, __LINE__);
        return -EINVAL;
    }
    //version format: KA030_A_1.0.4_XXX, 43+9-1
    get_ver_addr = 51;
    bat_fw_version = (int)(bat_fwinfo[get_ver_addr] & 0x0f) << 8 |
                (int)(bat_fwinfo[get_ver_addr + 2] & 0x0f) << 4 |
                (int)(bat_fwinfo[get_ver_addr + 4] & 0x0f);
    bin_fw_version = (int)(bin_fwinfo[get_ver_addr] & 0x0f) << 8 |
                (int)(bin_fwinfo[get_ver_addr + 2] & 0x0f) << 4 |
                (int)(bin_fwinfo[get_ver_addr + 4] & 0x0f);

    if (bat_fw_version != bin_fw_version) {
        kb_err("%s %d bin_fw_version != bin_fw_version, please check!!!\n", __func__, __LINE__);
        return -EINVAL;
    }
    pogo_keyboard_client->kpdmcu_fw_data_ver = bat_fw_version;
    ...
    //calculate bat file crc32
    crc32 = dfu_crc32(&fw_data[bat_start_addr], bat_len, &crc32);
    kb_debug("%s %d bat file checksum: 0x%x\n", __func__, __LINE__, crc32);
    *checksum1 = crc32;
    //calculate bin file crc32
    crc32 = 0;
    crc32 = dfu_crc32(&fw_data[bin_start_addr], bin_len, &crc32);
    kb_debug("%s %d bin file checksum: 0x%x\n", __func__, __LINE__, crc32);
...
static void kpdmcu_fw_data_version_thread(struct work_struct *work)
{
    ...
    if(fw_entry != NULL) {
        ...
        if (pogo_keyboard_client->pogopin_ota_dfu)
            ret = kpd_fw_dfu_isvalid(fw_entry->data, fw_entry->size, &checksum1, &bat_start_addr, &bat_len,
                                &bat_type, &checksum2, &bin_start_addr, &bin_len, &bin_type);
        else
...
static void kpdmcu_fw_update_thread(struct work_struct *work)
{
    ...
    if(pogo_keyboard_client->is_kpdmcu_need_fw_update == false && pogo_keyboard_client->kpdmcu_fw_update_force == false) {
        ...
        if(fw_entry != NULL) {
            ...
            if (pogo_keyboard_client->pogopin_ota_dfu)
                ret = kpd_fw_dfu_isvalid(firmware_data, fw_data_count, &checksum1, &bat_start_addr, &bat_len,
                                    &bat_type, &checksum2, &bin_start_addr, &bin_len, &bin_type);
            else
        ...
        if (pogo_keyboard_client->pogopin_ota_dfu) {
            ret = kpd_fw_dfu_update(firmware_data, fw_data_count, checksum1, bat_start_addr, bat_len, bat_type);
            if (ret == 0) {
                pogo_keyboard_client->fw_update_progress = FW_PROGRESS_50 * FW_PERCENTAGE_100;
                ret = kpd_fw_dfu_update(firmware_data, fw_data_count, checksum2, bin_start_addr, bin_len, bin_type);
                if (ret == 0) {
                    max_disconnect_count = 160; //8s
                    dfu_boot = 1;
                    //dfu boot need > 6s
                    do {
                        ...
                        (pogo_keyboard_client->fw_update_progress >= FW_PROGRESS_96 * FW_PERCENTAGE_100));
                    if (pogo_keyboard_client->fw_update_progress > 1) {
                        pogo_keyboard_client->kpd_fw_status = FW_UPDATE_FAIL;
                        pogo_keyboard_client->fw_update_progress = 0;
                        goto out1;
                    }
